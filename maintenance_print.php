<?php
    ...
    $pdf->setY($pdf->gety());
    $y=$pdf->gety();
    //$pdf->Multicell(0,4,'Thank you for your inquiry dated '.date('jS F Y',strtotime($data['order_date'])).', we are pleased to quote as follows : ',0,'L');
    $pdf->Multicell(0,4,$data['order_regards'],0,'L');
    $pdf->setY($pdf->gety()+2);
...
$term = "We trust that you will find our quotation acceptable and look forward to receive your confirmation soon.";
                ...
                $pdf->multicell(0,5,$data['remarks_desc'],0,'TL');
...
/*$term = "
DELIVERY \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t: ".$data['order_delivery_remark']."
PRICE \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t: ".$data['order_price_remark']."
PAYMENT TERMS \t\t\t\t\t\t\t : ".$data['order_paymentterm_remark']."
VALIDITY \t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t: ".$data['order_validity_remark']."
COUNTRY OF ORIGIN \t: ".$data['order_country']."
*/							
