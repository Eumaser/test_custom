<?php
    ...
    $o->document_url = "report.php?type=$o->type";
    $o->document_print_url = "report_print.php?type=$o->type";
    ...
    $whereSql = "WHERE inv.invoice_status = 1 AND (inv.invoice_prefix_type = 'SI' OR inv.invoice_prefix_type = 'eSI')";
    $orderSql = '';
    
    if(!empty($o->report_invoice)){
        $whereSql .= " AND inv.invoice_no LIKE '%$o->report_invoice%' ";
    }    
    ...
    if(!empty($o->report_date_from) && !empty($o->report_date_to)){
        $whereSql .= " AND inv.invoice_date BETWEEN '".date('Y-m-d', strtotime($o->report_date_from))."' AND '".date('Y-m-d', strtotime($o->report_date_to))."' ";
    }else if(!empty($o->report_date_from)){
        $whereSql .= " AND inv.invoice_date >= '".date('Y-m-d', strtotime($o->report_date_from))."'";
    }else if(!empty($o->report_date_to)){
        $whereSql .= " AND inv.invoice_date <= '".date('Y-m-d', strtotime($o->report_date_to))."'";
    }
    
    if(!empty($o->report_product_id)){
        $whereSql .= " AND invl.invl_pro_id = '$o->report_product_id'";
    }
    /*
    if(!empty($o->detailed_invoice)){
        $whereSql .= " AND inv.invoice_no LIKE '%$o->detailed_invoice%' ";
    }  
    ...
    if(!empty($o->detailed_date_from) && !empty($o->detailed_date_to)){
        $whereSql .= " AND inv.invoice_date BETWEEN '".date('Y-m-d', strtotime($o->detailed_date_from))."' AND '".date('Y-m-d', strtotime($o->detailed_date_to))."' ";
    }else if(!empty($o->detailed_date_from)){
        $whereSql .= " AND inv.invoice_date >= '".date('Y-m-d', strtotime($o->detailed_date_from))."'";
    }else if(!empty($o->detailed_date_to)){
        $whereSql .= " AND inv.invoice_date <= '".date('Y-m-d', strtotime($o->detailed_date_to))."'";
    }
    */
