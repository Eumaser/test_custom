<?php
    ...
    if($_REQUEST['action'] == 'PO'){
        $report_name = 'Purchase Order';
    }else{
        $report_name = 'Request Form';
    }
    ...
    $pdf->setx(25);
    $pdf->SetFont($font_name,'B',10);
    $pdf->Cell(2,4,':  '.$data['order_attentionto_name'].' ('.$data['order_attentionto_email'].')',0,1,'L');
    ...
    $pdf->Multicell(0,4,'RE: '.strtoupper($report_name).' NO. '.$data["{$data_field_type}_no"],0,'L');
    
    $pdf->SetFont($font_name,'',10);
    $pdf->ln(2);
    $pdf->setY($pdf->gety());
    $y=$pdf->gety();
    $pdf->Cell(2,4,$data["{$data_field_type}_regards"],0,1,'L');
    $pdf->setY($pdf->gety()+2);
    ...
    $pdf->setY($y+1);
    $pdf->SetFont($font_name,'B',10);
    $pdf->cell(15,4,"No",$border,'','L');
    $pdf->cell(25,4,"Part No",$border,'','L');
    $pdf->cell(65,4,"Description",$border,'','C');
    if($_REQUEST['format'] == 1){
        $pdf->cell(22,4,"Qty",$border,'','C');	
    }else{
        $pdf->cell(22,4,"",$border,'','C');	
    }
    $pdf->cell(28,4,"U/Price",$border,'','C');
    if($_REQUEST['format'] == 1){
        $pdf->cell(28,4,"Total Amt (US$)",$border,'','C');
    }else{
        $pdf->cell(28,4,"",$border,'','C');
    }
    $pdf->setY($pdf->GetY());
    ...
    while($row = mysql_fetch_array($query)){
        ...
        if( ($A4Y-($y+10+$pdf->getStringHeight(80, $row["{$data_field_line_type}_pro_desc"]))) < 20){
            ...
            $yg = $pdf->GetY();
            $pdf->SetFont($font_name,'B',10);
            $pdf->cell(15,4,"No",$border,'','C');
            $pdf->cell(25,4,"Part No",$border,'','C');
            $pdf->cell(65,4,"Description",$border,'','C');
            if($_REQUEST['format'] == 1){
                $pdf->cell(22,4,"Qty",$border,'','C');	
            }else{
            ...
            $pdf->cell(28,4,"U/Price",$border,'','C');
            if($_REQUEST['format'] == 1){
                $pdf->cell(25,4,"Total Amt (US$)",1,'','C');
                //$pdf->multicell(28,4,"Total Amt (US$)",$border,'C');
            }else{
                $pdf->cell(28,4,"",$border,'','C');
            }
            // Line break
        ...
        $pdf->SetY($y);
        $pdf->multicell($wc=15,5,$i,$border,'L');


        $sql = "SELECT product_part_no FROM db_product WHERE product_id = '".$row['ordl_pro_id']."' LIMIT 1 "; 
        $q = mysql_query($sql);
        while($item_code = mysql_fetch_array($q)){
            $pdf->SetY($y);
            $pdf->SetX($x=$x+$wc);
            $pdf->multicell($wc=25,5,htmlspecialchars_decode($item_code['product_part_no']),$border,'L');
        }
                  
        $pdf->SetY($y);
        $pdf->SetX($x=$x+$wc);

        $pdf->multicell($wc=65,5,htmlspecialchars_decode(htmlspecialchars_decode($row["{$data_field_line_type}_pro_desc"])),$border,'TL');
        $offsety1=$pdf->gety();

        $pdf->SetY($y);
        $pdf->SetX($x=$x+$wc);
        $pdf->multicell($wc=22,5,$row["{$data_field_line_type}_qty"],$border,'C'); //$row["uom_code"]
        $pdf->SetY($y);
        $pdf->SetX($x=$x+$wc);
        $pdf->multicell($wc=25,5, num_format($row["ordl_fuprice"],$round_type),$border,'R');
        $pdf->SetY($y);
        $pdf->SetX($x=$x+$wc);
        $pdf->multicell($wc=28,5, num_format(($row["ordl_fuprice"]*$row["{$data_field_line_type}_qty"])-$row["ordl_fdiscamt"],$round_type),$border,'R');
        $yoffset=$pdf->gety();
    ...
    if($_REQUEST['format'] == 1){
        ...
        $pdf->Cell(23,6,'Total: ',0,0,'R',false);
        $pdf->setx(($w*2)-34);
        ...
        $pdf->Cell(35,6,'US$' . num_format((($subtotal-$disctotal)-$data['order_discheadertotal']),$round_type),0,0,'R',false); 
        ...
        $pdf->setx(($w*2)-60);
        $pdf->SetFont($font_name,'B',10);
        $pdf->Cell(23,6,'Sub Total: ',0,0,'R',false);
        $pdf->setx(($w*2)-34);
        $pdf->SetFont($font_name,'',10);
        $pdf->Cell(35,6,'US$ ' . num_format((($subtotal-$disctotal)-$data['invoice_discheadertotal']),$round_type),0,0,'R',false); 
        ...
        $pdf->setx(($w*2)-60);
        $pdf->SetFont($font_name,'B',10);
        $pdf->Cell(23,6,'GST 7%: ',0,0,'R',false);
        $pdf->setx(($w*2)-34);
        $pdf->SetFont($font_name,'',10);
        $pdf->Cell(35,6,'$' . num_format((($subtotal-$disctotal)-$data['invoice_discheadertotal'])*(system_gst_percent/100),$round_type),0,0,'R',false); 
        ...
        $pdf->setx(($w*2)-60);
        $pdf->SetFont($font_name,'B',12);
        $pdf->Cell(23,6,'Total (NETT): ',0,0,'R',false);
        $pdf->setx(($w*2)-34);
        //$pdf->Cell(23,6,'$' . num_format((($subtotal-$disctotal)-$data['order_discheadertotal']) + (($subtotal-$disctotal)-$data['order_discheadertotal'])*(system_gst_percent/100),$round_type),0,0,'R',false); 
        $pdf->Cell(35,6,'US$ ' .num_format((($subtotal-$disctotal)-$data['invoice_discheadertotal'])+(($subtotal-$disctotal)-$data['invoice_discheadertotal'])*(system_gst_percent/100),$round_type),0,0,'R',false); 
...
//        $pdf->sety($yg);	
//    }
    $term1 = "Please advise the weight & dimension of our orders when ready. We will advise our shipping instructions later.";
    $term2 = "Awaiting your prompt acknowledgement.";
/*
    $pdf->SetFont($font_name,'B',10);
    $pdf->ln(2);
    $pdf->setY($pdf->gety());
...
*/
    ...
    $y = $pdf->gety();
    $pdf->SetY($y+5);
    $pdf->multicell(0,5,htmlspecialchars_decode($data["remarks_desc"]),0,'TL');

//    $pdf->ln(3);
//    $y = $pdf->gety();
//    $pdf->SetY($y+3);
//    $pdf->multicell(0,5,$term2,0,'TL');
...
//    $pdf->multicell(100,5,'Best Regards',0,'TL');
    ...
    $pdf->SetFont($font_name,'I',10);
    $pdf->SetY($y);
    $pdf->multicell(50,5,'Tel: '.$com_info['cprofile_tel'],0,'TL');
    $y = $pdf->gety();
    $pdf->SetFont($font_name,'I',10);
    $pdf->SetY($y);
    $pdf->multicell(50,5,'Fax: '.$com_info['cprofile_fax'],0,'TL');
