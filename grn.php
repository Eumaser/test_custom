<?php
    ...
    $o->save = $s;
    $o->document_type = 'GRN';
    $o->document_name = 'Goods Received Note Management';
    $o->document_code = 'Goods Received Note';
    $o->document_url = 'grn.php';
    $o->document_print_url = 'grn_print.php';
   ...
   switch($action){
       ...
       case "saveline":
       case "updateline":   
            //we find the project limit amount
            $p->project_id = $o->order_project_id;
            $r = $p->getProjectDetailTransaction();
            $o->calculateLineAmount();
            
            //Current Ordl amount
            $ordl_amount = $o->ordl_ftotal;
            
            //we find back original line amount 
            //$rl = $o->fetchOrderLineDetail(" AND ordl_id = '$o->ordl_id'","","",2);
            //$ordl_original_amount = $rl['ordl_ftotal'];

            //we get this transaction detail
            //$ro = $o->fetchOrderDetail(" AND order_id = '$o->order_id'","","",2);
            //we find back total already PO amount
            //$already_po_total = $o->getProjectTotalPOAmount(" AND o.order_project_id = '{$ro['order_project_id']}'");

            //use total PO amount - original line amount + current line amount , then we can know it have over limit or not
            //$total = ($already_po_total - $ordl_original_amount) + $ordl_amount;

            //do checking , if over limit then we false it.
            //if($total > $r['project_limit']){
            //    echo json_encode(array('status'=>0,'msg'=>$language[$lang]['project_over_limit']));
            //    exit();
            //}
