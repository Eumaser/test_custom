...
class AutoComplete {
    ...
    public function getItemAutoComplete($term){
        ...
        $order_project_id = $_REQUEST['order_project_id'];
        if($category_id > 0){
        ...
        if(($_REQUEST['document_type'] == 'QT') || ($_REQUEST['document_type'] == 'SI') || ($_REQUEST['document_type'] == 'SCN') || ($_REQUEST['document_type'] == 'SO')){
//            COALESCE(ms.iscategory_code,''),' - ',COALESCE(mss.isscategory_code,''),' - ',i.product_code) as product_code
        $sql = <<<EOF
           
                SELECT 'product' as itype,i.product_id,i.product_code,
                i.product_code as org_code
                FROM db_product i 
                LEFT JOIN db_isscategory mss ON i.product_category = mss.isscategory_id
                LEFT JOIN db_iscategory ms ON ms.iscategory_id = mss.issparent_id
                LEFT JOIN db_category mc ON mc.category_id = ms.isparent_id
                WHERE i.product_code LIKE '%$term%' $wherestring AND i.product_status = 1 
                
                
EOF;
        }else{
            if($order_project_id > 0){
                $sql = "SELECT pro.probom_material_id
                        FROM db_proaward pa 
                        INNER JOIN db_product i ON i.product_id = pa.proaward_ordl_id
                        INNER JOIN db_probom pro ON pro.probom_product_id = i.product_id
                        WHERE i.product_code LIKE '%$term%' AND pa.proaward_project = 1 ";
//                $query = mysql_query($sql);
                
            }
            $sql = "

                    SELECT 'material' as itype,m.material_id as product_id,CONCAT(COALESCE(mc.materialcategory_code,''),' - ',COALESCE(ms.mscategory_code,''),' - ',COALESCE(mss.msscategory_code,''),' - ',m.material_code) as product_code,
                    m.material_code as org_code
                    FROM db_material m 
                    ...
                    WHERE CONCAT(COALESCE(mc.materialcategory_code,''),' - ',COALESCE(ms.mscategory_code,''),' - ',COALESCE(mss.msscategory_code,''),' - ',m.material_code) LIKE '%$term%' AND m.material_status = 1  AND
                    m.material_id IN (SELECT pro.probom_material_id
                        FROM db_proaward pa 
                        INNER JOIN db_ordl ordl ON ordl.ordl_id = pa.proaward_ordl_id
                        INNER JOIN db_probom pro ON pro.probom_product_id = ordl.ordl_pro_id
                        WHERE pa.proaward_project = 1 )
...
//                LEFT JOIN db_mscategory ms ON ms.mscategory_id = mss.mssparent_id
//                LEFT JOIN db_materialcategory mc ON ms.msparent_id = mc.materialcategory_id
//                WHERE CONCAT(COALESCE(mc.materialcategory_code,''),' - ',COALESCE(ms.mscategory_code,''),' - ',COALESCE(mss.msscategory_code,''),' - ',m.material_code) LIKE '%$term%' AND m.material_status = 1  
//                   
//                $subsql
//                ";
        ...
        while($row = mysql_fetch_array($query)){
            ...
            if($row['itype'] == 'product'){
                if($row['product_id'] == 3){// free text
                    $product_code = $row['org_code'];
                }
            }
            $results[] = array('id'=>$row['product_id'],'value'=>$product_code,'text'=>$product_code,'itype'=>$row['itype']);
        }
        if(sizeof($results) <=0){
