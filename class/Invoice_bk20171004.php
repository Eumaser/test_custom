...
class Invoice {
    ...
    public function createInvoice(){
        ...
        $table_field = array('invoice_no','invoice_date','invoice_customer','invoice_salesperson',
                             ...
                             'invoice_fax','invoice_subcon','invoice_project_id','invoice_shipping_id',
                             ...
                             'invoice_attentionto_name','invoice_tnc','invoice_regards'
                             );
        $table_value = array($invoice_no,format_date_database($this->invoice_date),$this->invoice_customer,$this->invoice_salesperson,
                             ...
                             $this->invoice_fax,$this->invoice_subcon,$this->invoice_project_id,$this->invoice_shipping_id,
                             ...
                             $this->invoice_attentionto_name,$this->invoice_term_remark,$this->invoice_regards
                             );
    ...
    public function updateInvoice(){
        $table_field = array('invoice_date','invoice_customer','invoice_salesperson',
                             ...
                             'invoice_project_id','invoice_shipping_id',
                             'invoice_paymentterm_id','invoice_delivery_id','invoice_price_id','invoice_validity_id',
                             'invoice_transittime_id','invoice_freightcharge_id','invoice_pointofdelivery_id','invoice_prefix_id',
                             'invoice_remarks_id','invoice_country_id','invoice_attentionto_email',
                             'invoice_attentionto_name','invoice_tnc','invoice_regards');
        $table_value = array(format_date_database($this->invoice_date),$this->invoice_customer,$this->invoice_salesperson,
                             ...
                             $this->invoice_project_id,$this->invoice_shipping_id,
                             $this->invoice_paymentterm_id,$this->invoice_delivery_id,$this->invoice_price_id,$this->invoice_validity_id,
                             $this->invoice_transittime_id,$this->invoice_freightcharge_id,$this->invoice_pointofdelivery_id,$this->invoice_prefix_id,
                             $this->invoice_remarks_id,$this->invoice_country_id,$this->invoice_attentionto_email,
                             $this->invoice_attentionto_name,$this->invoice_term_remark,$this->invoice_regards);
    ...
    public function updateInvoiceLine(){
        ...
        if(!$this->save->UpdateData($table_field,$table_value,'db_invl','invl_id',$remark,$this->invl_id)){
           ...
           if($this->document_type == 'PCN'){
               if(!$this->updateStockTransaction($this->invl_id,'out')){
                   return false;
               }
               else{
                   return true;
               ...
               if(!$this->updateStockTransaction($this->invl_id,'in')){
                   return false;
               }
               else{
                   return true;
...
//        }
    ...
    public function fetchPrevInvoiceLineDetail($wherestring,$orderstring,$wherelimit,$type){
        $sql = "SELECT * 
                FROM db_stock_transaction
                WHERE (ref_id > 0 AND item_id > 0)
                $wherestring $orderstring $wherelimit";
        ...
        if($type == 1){
            ...
            $this->trans_invl_id        = $row['documentline_id'];
            $this->trans_invl_order_id  = $row['ref_id'];
            $this->trans_invl_qty       = $row['quantity'];
            $this->trans_invl_type      = $row['type'];
            $this->trans_invl_pro_id    = $row['item_id'];
            $this->trans_invl_cust_id   = $row['custsupp_id'];
            $this->trans_invl_order_date    = $row['document_date'];
        }else if($type == 2){
            return mysql_fetch_array($query);
        }
        return $query;
                ...
                <form id = 'invoice_form' class="form-horizontal" action = '<?php echo $this->document_url;?>?action=create' method = "POST">
                  <div class="box-body col-sm-9">
                        <div class="form-group">
                            ...
                            <label for="invoice_date" class="<?php echo $label_col_sm;?> control-label"><?php echo $this->document_code;?> Date</label>
                            <div class="<?php echo $field_col_sm;?>">
                                <input type="text" class="form-control datepicker" id="invoice_date" name="invoice_date" value = "<?php echo format_date($this->invoice_date);?>" placeholder=" <?php echo $this->document_code;?> Date">
                            </div>
                        </div>  
                        <!--
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="<?php echo $field_col_sm;?>">
                                 <select class="form-control select2" id="invoice_customer" name="invoice_customer" >
                                     <?php echo $this->customerCrtl;?>
                                 </select>
                            </div>
                            <label for="invoice_attentionto" class="<?php echo $label_col_sm;?> control-label">Attention To</label>
                            <div class="col-sm-3">
                                <select class="form-control select2" id="invoice_attentionto" name="invoice_attentionto">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-3">
                                  <textarea class="form-control" rows="3" id="invoice_billaddress" name="invoice_billaddress" placeholder="Billing Address"><?php echo $this->invoice_billaddress;?></textarea>
                            </div>
                            <label for="invoice_shipaddress" class="<?php echo $label_col_sm;?> control-label">Shipping Address</label>
                            <div class="<?php echo $field_col_sm;?>">
                                    <textarea class="form-control" rows="3" id="invoice_shipaddress" name="invoice_shipaddress" placeholder="Shipping Address"><?php echo $this->invoice_shipaddress;?></textarea>
                            </div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                            <label for="invoice_remark" class="<?php echo $label_col_sm;?> control-label">Notes</label>
                            <div class="<?php echo $field_col_sm;?>">
                                <textarea class="form-control" rows="3" id="invoice_remark" name="invoice_remark" placeholder="Notes" <?php echo $disabled;?>><?php echo $this->invoice_remark;?></textarea>
                            </div> 
                            <label for="invoice_regards" class="<?php echo $label_col_sm;?> control-label">Regards</label>
                            <div class="<?php echo $field_col_sm;?>">
                                <textarea class="form-control" rows="3" id="invoice_regards" name="invoice_regards" placeholder="Regards" <?php echo $disabled;?>><?php echo $this->invoice_regards;?></textarea>
                            </div>
                        </div>
...
//                    '<td style = "width:60px;"><input type = "text" id = "invl_seqno_@i" class="form-control" value=""/></td>'+
                    ...
                    '<td style = "width:300px;"><textarea rows="1" id = "invl_pro_desc_@i" class="form-control"></textarea></td>'+
...
//                    '<td style = "width:20px;"><input type = "checkbox" style = "width:20%" id = "invl_istax_@i" class = "minimal isincludetax"/></td>'+
    ...
    $(document).ready(function() {
        <?php if($this->document_type != 'SI' && $this->document_type != 'SCN' && $this->document_type != 'PCN'){ ?>
            addline(); 
        <?php }else if($this->document_type == 'PCN' || $this->document_type == 'SCN'){
                    if($this->invoice_generate_from == 0 && $this->invoice_generate_from_type == null){
        ?> 
                addline();
         <?php       }     
                }?>
  ...
  <body class="hold-transition skin-blue layout-top-nav">
        ...
        <tbody>
            ...
            ?>
                <tr id = "line_<?php echo $line;?>" class="tbl_grid_odd" line = "<?php echo $line;?>">
                    ...
                    <td style="width:400px;"><textarea rows="1" cols="15" id = "invl_pro_desc_<?php echo $line;?>" class="form-control" <?php echo $readonly;?>><?php echo $row['invl_pro_desc'];?></textarea></td>
                    ...
                    <td style="width:500px;" ><textarea rows="1" width="500px" id = "invl_pro_remark_<?php echo $line;?>" class="form-control" <?php echo $readonly;?>><?php echo $row['invl_pro_remark'];?></textarea></td>
    ...
    public function generateInvoice($r){
        ...
        $table_value = array($invoice_no,system_date,$r[$db_tbl.'_customer'],$r[$db_tbl.'_salesperson'],
                             ...
                             $r[$db_tbl.'_regards'],$r[$db_tbl.'_tnc']);
        $remark = "Insert $this->document_code.<br> Document No : $invoice_no";
        if(!$this->save->SaveData($table_field,$table_value,'db_invoice','invoice_id',$remark)){
           return false;
        }else{
    ...
    public function generateInvoiceLine($r,$invoice_id){
        ...
        $table_field = array('invl_invoice_id','invl_pro_id','invl_pro_desc','invl_qty','invl_uom',
                             ...
                             'invl_parent_type','invl_pro_remark','invl_item_type');
        $table_value = array($invoice_id,$r[$db_tbl.'_pro_id'],$r[$db_tbl.'_pro_desc'],$r[$db_tbl.'_qty'],$r[$db_tbl.'_uom'],
                             ...
                             $line_parent_type,$r[$db_tbl.'_pro_remark'],$r[$db_tbl.'_item_type']);
    ...
    public function updateStockTransaction($invl_id,$action){
        include_once 'Product.php';
        include_once 'Package.php';
        $p = new Product();   
        $g = new Package();   
        ...
        if($this->invl_item_type == 'product'){
            $p->fetchProductDetail(" AND product_id = '$this->invl_pro_id'","","",1);
            $product_qty = $this->invl_qty;
            $this->fetchPrevInvoiceLineDetail(" AND documentline_id = '$invl_id' AND item_id = '$this->invl_pro_id' AND type='$action'","","",1);
            $prev_qty = $this->trans_invl_qty;
            if($action=='out'){            
            ...
            while($row = mysql_fetch_array($query)){
                $p->fetchProductDetail(" AND product_id = '".$row['package_product_id']."'","","",1);
                $product_qty = $this->invl_qty * $row['package_product_qty'];                
                $this->fetchPrevInvoiceLineDetail(" AND documentline_id = '$invl_id' AND item_id = '".$row['package_product_id']."' AND type='$action'","","",1);
                $prev_qty = $this->trans_invl_qty;
                if($action=='out'){            
                    $stock_balance = ($p->product_stock + $prev_qty) - $product_qty;
                    $stock_desc = 'OUT';
