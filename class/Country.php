...
class Country {
    ...
    public function create(){
        if(!empty($this->category) && $this->category == "item"){
            $db = "db_countryitem";
        }else{
            $db = "db_country";
        }
        ...
        if(!$this->save->SaveData($table_field,$table_value,$db,'country_id',$remark)){
           return false;
        }else{
    ...
    public function update(){
        ...
        if(!$this->save->UpdateData($table_field,$table_value,$db,'country_id',$remark,$this->country_id)){
           return false;
        }else{
           return true;
        }
    ...
    public function fetchCountryDetail($wherestring,$orderstring,$wherelimit,$type){
        ...
        $sql = "SELECT * FROM {$db} WHERE country_id > 0  $wherestring $orderstring $wherelimit";
    ...
    public function delete(){
        $db = (!empty($this->category) && $this->category == "item") ? "db_countryitem" : "db_country";
        
        //if($this->save->DeleteData("db_country"," WHERE country_id = '$this->country_id'","Delete Country.")){
        if($this->save->DeleteData($db," WHERE country_id = '$this->country_id'","Delete Country.")){
            return true;
        }else{
            return false;
        }
          ...
          <section class="content">
            <div class="box box-success">
              <div class="box-header with-border">
                ...
                <button type = "button" class="btn btn-primary pull-right" style = 'width:150px;' onclick = "window.location.href='country.php?cat=<?php echo $cat;?>'">Search</button>
                <?php if(getWindowPermission($_SESSION['m'][$_SESSION['empl_id']],'create')){?>
                <button type = "button" class="btn btn-primary pull-right" style = 'width:150px;margin-right:10px;' onclick = "window.location.href='country.php?action=createForm&cat=<?php echo $cat;?>'">Create New</button>
                <?php }?>
              </div>
                ...
                <form id = 'country_form' class="form-horizontal" action = 'country.php?action=create' method = "POST">
                  ...
                  <div class="box-footer">
                    ...
                    <input type = "hidden" value = "<?php echo $this->category;?>" name = "cat"/>
                    <?php 
          ...
          <div class="row">
            <div class="col-xs-12">
              <div class="box">
                ...
                <?php if(getWindowPermission($_SESSION['m'][$_SESSION['empl_id']],'create')){?>
                <button class="btn btn-primary pull-right" onclick = "window.location.href='country.php?action=createForm&cat=<?php echo $cat;?>'">Create New + </button>
                <?php }?>
                </div><!-- /.box-header -->
                <div class="box-body">
                    ...
                    ?>
                        <tr>
                            ...
                            <td class = "text-align-right">
                                ...
                                ?>
                                <button type="button" class="btn btn-primary btn-info " onclick = "location.href = 'country.php?action=edit&country_id=<?php echo $row['country_id'];?>&cat=<?php echo $cat;?>'">Edit</button>
                                <?php }?>
                                <?php 
                                if(getWindowPermission($_SESSION['m'][$_SESSION['empl_id']],'delete')){
                                ?>
                                <button type="button" class="btn btn-primary btn-danger " onclick = "confirmAlertHref('country.php?action=delete&country_id=<?php echo $row['country_id'];?>&cat=<?php echo $cat;?>','Confirm Delete?')">Delete</button>
                                <?php }?>
                            </td>
                        </tr>
                    <?php    
