...
//        }
    ...
    public function getInputForm($action){
        ...
        //if($this->document_type == 'QT') {
            ...
            $this->countryCrtl = $this->select->getCountrySelectCtrl($this->order_country_id,'Y');
       // }
        ...
        if($this->document_type == 'QT'){
            $isgenerated = getDataCountBySql("db_invoice"," WHERE invoice_generate_from = '$this->order_id' AND invoice_status = 1 AND invoice_generate_from > 0");
        }else if($this->document_type == 'PO'){
            $isgenerated = getDataCountBySql("db_invoice"," WHERE invoice_generate_from = '$this->order_id' AND invoice_status = 1 AND invoice_generate_from > 0");
        }else if($this->document_type == 'DO'){
            $isgenerated = getDataCountBySql("db_order"," WHERE order_generate_from = '$this->order_id' AND order_status = 1 AND order_generate_from > 0");
        }
...
<div class="modal fade modal-wide" id="ProductDetailModal" role="dialog">
    ...
    var line_copy = '<tr id = "line_@i" class="tbl_grid_odd" line = "@i">' +
                    ...
                    <?php if(($this->document_type == 'PO' || $this->document_type == 'GRN')){?>
                    '<td style = "width:300px;"><textarea rows="1" id = "ordl_product_location_@i" class="form-control"></textarea></td>'+    
                    <?php }?>  
    ...
    <table id="detail_table" class="table transaction-detail">
        <thead>
          <tr>
            ...
            <?php } ?>
            <th class = "" style = 'width:100px;'>Part No</th>
...
<!-- Subtotal    --><td style = "width:100px;"><input type = "text" id = "ordl_total_<?php echo $line;?>" class="form-control text-align-right" readonly value = "<?php echo num_format($row['ordl_total']);?>"/></td>
                    <?php }?>
                    <?php if(($this->document_type == 'PO' || $this->document_type == 'GRN')){?>
<!-- Location    --><td style="width:300px;"><textarea rows="1" cols="15" line = "<?php echo $line;?>" id = "ordl_product_location_<?php echo $line;?>" class="form-control text-align-left" <?php echo $readonly;?>><?php echo $row['ordl_product_location'];?></textarea></td>
                    <?php }?>
                    <?php if(($this->document_type == 'PRF')){?>
...
o               WHERE  $wherestring"; 
    ...
    ?>
        <div class="form-group">
            ...
            <?php if($this->document_type == 'PO' || $this->document_type == 'GRN' || $this->document_type == 'DO' || $this->document_type == 'PU') { ?>
            <div class="form-group">
        ...
        <div class="form-group">
            ...
            <?php if($this->document_type != 'PRF'){?>
            <label for="order_regards" class="<?php echo $label_col_sm;?> control-label">Notes to Customer</label>
            <div class="<?php echo $field_col_sm;?>">
                <textarea class="form-control" rows="3" id="order_regards" name="order_regards" placeholder="Notes to Customer" <?php echo $disabled;?>><?php echo $this->order_regards;?></textarea>
            </div>
            <?php }?>
        </div>
    <?php
