...
//echo $sql;die;
    ...
    public function getProductSelectCtrl($pid,$shownull="Y",$wherestring=''){
        $sql = "SELECT product_id,product_part_no,product_sale_price,product_cost_price,product_name
                FROM db_product m
    ...
    public function getProductNameSelectCtrl($pid,$shownull="Y",$wherestring=''){
        $sql = "SELECT product_id,product_part_no,product_name
                FROM db_product
                WHERE (product_id = '$pid' or product_id >0) and product_status = 1 $wherestring
                ORDER BY product_part_no ASC";
        ...
        while($row = mysql_fetch_array($query)){
            ...
            $selectctrl .='<option value = "'.$id.'" qt-type="product" '.$selected.'>'.$row['product_part_no'].' - '.$row['product_name'].'</option>';
        }
        return $selectctrl;
    }
    ...
    public function getCountryItemSelectCtrl($pid,$shownull="Y",$wherestring=''){
        $sql = "SELECT country_id,country_code from db_countryitem WHERE (country_id = '$pid' or country_id >0) and country_status = 1 $wherestring
                ORDER BY country_seqno,country_code ASC";
        if($shownull =="Y"){
            $selectctrl .="<option value = '' SELECTED='SELECTED'>Select One</option>";
        }
