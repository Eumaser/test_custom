...
class SavehandlerApi {
    ...
    public function UpdateData($table_field,$table_value,$table,$table_id,$remark,$update_id){
        global $sql_debug;
        if($table == "" || $update_id <= 0){
            return false;
        }
       $date_time = date('Y-m-d H:i:s');
       ...
       $sql_insert = "UPDATE $table SET $v WHERE $table_id = '$update_id'";
       if($sql_debug){
           echo $sql_insert;die;
       }
       if(mysql_query($sql_insert)){
